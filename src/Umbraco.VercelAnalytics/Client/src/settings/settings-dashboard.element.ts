...
export class VercelAnalyticsSettingsDashboardElement extends UmbElementMixin(LitElement) {
  ...
  async #testConnection(key: string): Promise<void> {
    ...
    const { data, error } = await UmbracoVercelAnalyticsService.testConnection({ path: { key } });
      ...
      <form @submit=${this.#save} novalidate>
        ...
        ${this._dirty ? html`
          <div class="save-bar" aria-label="Unsaved Vercel Analytics settings">
            ...
            <uui-button type="submit" look="primary" label="Save Vercel Analytics settings" .state=${this._saving ? "waiting" : undefined} ?disabled=${this._saving}>Save settings</uui-button>
          </div>
        ` : ""}
