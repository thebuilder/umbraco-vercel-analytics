...
{
    private const string InitializationKey = "Umbraco.VercelAnalytics.AdministratorSectionAccess.v1";
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning("Could not grant the Vercel Analytics section because the Administrators group was not found.");
            return;
        }
        ...
        if (ShouldGrantAnalyticsSection(administrators.AllowedSections))
        {
            ...
            if (!result.Success)
            {
                logger.LogWarning("Could not grant the Vercel Analytics section to the Administrators group.");
                return;
            }
        }
